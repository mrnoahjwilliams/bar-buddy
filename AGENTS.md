...
- Follow Workflow's standing authority: commits, feature-branch pushes, and PRs are allowed for requested work; the user approves merges, which automatically deploy under Workflow’s continuous-delivery policy.
...
- Report changes, actual verification, Git/PR status, blockers, and the concrete next user instruction briefly. Open a complete PR and stop under [Workflow's CI handoff rule](docs/07-development-workflow.md#finish-and-review); the user waits for checks and merges directly in GitHub, triggering automatic deployment without a post-CI continuation; never invent passing checks or update completion based only on intent.
