...
4. Make focused commits, push the feature branch, and open/update a complete, non-draft PR using the template. Inspect CI once on the current PR commit and fix any already-known failures. If checks are pending, report that accurately and stop with the PR ready for GitHub review. The user waits for required checks and squash-merges directly in GitHub; no “continue after CI” prompt or agent sign-off is required. Do not leave routine documentation, checkboxes, review notes or PR readiness waiting on CI. Use GitHub check status for transient CI evidence rather than a later docs-only commit. Use a draft only for genuinely incomplete or blocked work. A later CI failure blocks merge and needs a fix in the same PR; it does not require a routine continuation when CI passes. If no remote exists, report that no PR exists.
...
6. A reviewed merge to `main` authorizes automatic production delivery under the contract below. Report implementation, actual local checks, current CI and PR status, and any real pre-merge operator requirements. End with “Review the PR in GitHub and merge after required checks pass.” Do not require an agent return after passing CI or merge. Confirm deployment from provider evidence when requested or when starting the next scope; CI success alone is not deployment.
...
| Automatic deployment of reviewed `main` merges | Authorized by the user on September 10, 2026; merging is the production approval. Manual deployments, paid resources and destructive data operations remain separately controlled |
...
Dependency updates are manual. Do not enable Dependabot version/security-update PRs or recreate the closed bot PRs. Vulnerability alerts remain enabled for review. For maintenance work, inspect vulnerability reports and compatibility, keep runtime definitions aligned with the supported runtime major, and evaluate major toolchain upgrades as coordinated changes. Include justified updates in a coherent maintenance scope; merge approval remains required.
...
| `frontend-checks` | One lockfile install followed by catalog validation, formatting/lint, TypeScript, behavioral tests, production build and Vercel packaging with a non-production origin, isolated OpenAPI generation and Orval drift verification; fail for missing, stale, modified or unexpected untracked generated artifacts |
...
Verify real passing and deliberate-failure cases on a temporary task-branch change, remove the deliberate failure, and confirm final checks pass and protection blocks failed checks where supported. No placeholder success jobs. Local success does not verify GitHub CI. Production delivery uses the provider integrations below; no production credentials belong in these test jobs.

## Continuous delivery

The normal loop is **Start development → complete PR → GitHub review and passing checks → squash merge → automatic deployment**. The agent selects the next coherent Plan scope, implements and verifies it, updates affected docs and opens the finished PR, then stops. The next development request fetches `origin/main`, verifies prerequisites and prior deployment, and performs safe merged-branch cleanup before beginning the next scope. Do not demand a post-CI continuation or agent-mediated merge.

Protected `main` remains the approval gate: both required checks, an up-to-date branch and resolved conversations are required. Do not bypass protection by pushing directly to `main`. Vercel's existing Git integration builds only `main` and promotes successful production builds to the canonical domain. Render's auto-synced Blueprint tracks `main` and uses `autoDeployTrigger: checksPass`. Provider credentials remain in their current services; no new Actions deployment secrets or paid services are needed. [Local development](08-local-development.md#continuous-delivery) owns operational settings and recovery.

Deployment takes build/startup time, usually minutes; it is not instantaneous and already-open browser tabs need a reload to receive new frontend code. Providers deploy independently. Keep API and database changes compatible with old and new clients; use additive migrations and staged removal. A failing backend rollout can coexist with a successful frontend rollout. Render health and both provider deployment states determine success, not CI alone. Do not merge another release over an unresolved production failure.

For schema changes or catalog updates, disclose and complete required backups/operator imports before marking the PR ready to merge; never hide required manual work behind the automatic-deploy promise. Destructive migrations or incompatible changes need a separately agreed rollout. Routine compatible code changes require only GitHub review and merge.

GitHub Releases mark accepted product milestones, not each deployment. Tag the verified merged application revision and describe the delivered capabilities and material operating limits. Release creation does not trigger production deployment. Do not tag an unmerged PR as a published release.

## Instruction verification

For routing/workflow changes, check representative requests: product question, “what's next?”, “what did you implement?”, implementation, review, and a fresh-chat “start development” → GitHub review/checks → GitHub merge → automatic deployment sequence. Verify they select necessary context, respect authority, and identify blockers without starting unrequested work. Confirm that “merge” includes verified local/remote branch cleanup without another user confirmation, while preserving branches with subsequent work and respecting tool denials. A fresh repository-aware agent check may be used when authorized; previous preparation passed one. Keep verification in the PR/commit evidence, not an application-documentation narrative.
